...
function renderTeamCard(container, team, isRaptors) {
  container.className = `team-card${isRaptors ? " team-card--raptors" : ""}`;
  container.innerHTML = "";
...
function renderCenter(game) {
  ...
  clock.textContent = formatClock(game.clock) || "--:--";
...
async function refresh() {
  ...
  if (state.status !== "ok") {
    ...
    renderFallback(state.error || "Waiting for Raptors game data...");
    return;
  }
  ...
  const raptorsIsHome = home.tricode === "TOR";

  renderTeamCard(awayCard, away, !raptorsIsHome);
  renderTeamCard(homeCard, home, raptorsIsHome);
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (zoomSelect) {
    const savedZoom = localStorage.getItem("raptors-zoom") || "1";
    zoomSelect.value = savedZoom;
    document.body.style.zoom = savedZoom;
    zoomSelect.addEventListener("change", () => {
      const value = zoomSelect.value || "1";
      document.body.style.zoom = value;
      localStorage.setItem("raptors-zoom", value);
    });
  }
